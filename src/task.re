...
and task_seg_id = (user_id, int64, int64, int64, option(int64));
...
and recur_type =
  ...
  | Time_pattern_match(Time_pattern.time_pattern, recur_data)

and recur = {
  excluded_time_slots: list(Time_slot.t),
  recur_type,
}

and sched_req_template_data_unit =
  Sched_req_data_unit_skeleton.t(task_seg_size, int64, Time_slot.t)

and sched_req_template = list(sched_req_template_data_unit)
...
and task_inst_data = {task_inst_type}

and task_inst_type =
  | Reminder
  | Reminder_quota_counting({quota: int64})
  | Passing
...
module Id = {
  let task_seg_id_with_first_sub_id =
      ((id1, id2, id3, id4, id5): task_seg_id): task_seg_id => (
    id1,
    id2,
    id3,
    id4,
  ...
  let init_task_seg_sub_id = ((id, len): task_seg): task_seg => (
    task_seg_id_with_first_sub_id(id),
    len,
  );
  ...
  let succ_task_seg_sub_id = ((id1, id2, id3, id4, id5): task_seg_id) => (
    ...
    id3,
    id4,
    Option.map(x => x +^ 1L, id5),
  );

  let string_of_user_id = (id: user_id) => Printf.sprintf("%Ld", id);

  let user_id_of_string = (s: string): result(user_id, unit) =>
  ...
  let task_id_of_string = (s: string): result(task_id, unit) =>
    try(
      Scanf.sscanf(s, "%Ld.%Ld", (id1, id2) =>
        [@implicit_arity] Ok(id1, id2)
      )
    ) {
    | _ => Error()
    };
  ...
  let task_inst_id_of_string = (s: string): result(task_inst_id, unit) =>
    try(
      Scanf.sscanf(s, "%Ld.%Ld.%Ld", (id1, id2, id3) =>
        [@implicit_arity] Ok(id1, id2, id3)
      )
    ) {
    | _ => Error()
    };
  ...
  let task_seg_id_of_string = (s: string): result(task_seg_id, unit) =>
    try(
      Scanf.sscanf(s, "%Ld.%Ld.%Ld.%Ld.X", (id1, id2, id3, id4) =>
        [@implicit_arity] Ok(id1, id2, id3, id4, None)
      )
    ) {
    | _ =>
      try(
        Scanf.sscanf(s, "%Ld.%Ld.%Ld.%Ld.%Ld", (id1, id2, id3, id4, id5) =>
          [@implicit_arity] Ok(id1, id2, id3, id4, Some(id5))
        )
      ) {
      | _ => Error()
      }
  ...
  let task_id_of_task_seg_id =
      ((id1, id2, _id3, _id4, _id5): task_seg_id): task_id => (
    id1,
    id2,
  );

  let task_inst_id_of_task_seg_id =
      ((id1, id2, id3, _id4, _id5): task_seg_id): task_inst_id => (
    id1,
    id2,
    id3,
  );

  let task_inst_id_matches_task_id =
      (task_inst_id: task_inst_id, task_id: task_id): bool =>
    task_id == task_id_of_task_inst_id(task_inst_id);

  let task_seg_id_matches_task_id =
      (task_seg_id: task_seg_id, task_id: task_id): bool =>
    task_id == task_id_of_task_seg_id(task_seg_id);

  let task_seg_id_matches_task_inst_id =
      (task_seg_id: task_seg_id, task_inst_id: task_inst_id): bool =>
...
let task_seg_alloc_req_sum_length = reqs =>
  List.fold_left((acc, (_, size)) => acc +^ size, 0L, reqs);

let sched_req_template_bound_on_start_and_end_exc =
    (sched_req_template: sched_req_template): option((int64, int64)) =>
  List.fold_left(
    (acc, req_template_data_unit) => {
      let cur =
        switch (req_template_data_unit) {
        | Sched_req_data_unit_skeleton.Fixed({
            task_seg_related_data: task_seg_size,
            start,
          }) =>
...
module Serialize = {
  ...
  and pack_recur_type = (recur_type: recur_type): Task_t.recur_type =>
    switch (recur_type) {
    | [@implicit_arity] Arithemtic_seq(arith_seq, recur_data) =>
      `Arithmetic_seq((
        pack_arith_seq(arith_seq),
        pack_recur_data(recur_data),
      ))
    | [@implicit_arity] Time_pattern_match(pattern, recur_data) =>
      `Time_pattern_match((
  ...
  and pack_sched_req_template =
      (sched_req_template: sched_req_template): Task_t.sched_req_template =>
  ...
  and pack_task_inst = ((id, data): task_inst): Task_t.task_inst => (
    pack_task_inst_id(id),
    pack_task_inst_data(data),
  )

  and pack_task_inst_data =
      (task_inst_data: task_inst_data): Task_t.task_inst_data => {
    task_inst_type: pack_task_inst_type(task_inst_data.task_inst_type),
  }

  and pack_task_inst_type =
      (task_inst_type: task_inst_type): Task_t.task_inst_type =>
    switch (task_inst_type) {
    | Reminder => `Reminder
  ...
  and pack_task_seg = ((id, size)) => (
    pack_task_seg_id(id),
    Misc_utils.int32_int32_of_int64(size),
  )
...
module Deserialize = {
  ...
  and unpack_task_inst = ((id, data): Task_t.task_inst): task_inst => (
    unpack_task_inst_id(id),
    unpack_task_inst_data(data),
  )

  and unpack_task_inst_data =
      (task_inst_data: Task_t.task_inst_data): task_inst_data => {
    task_inst_type: unpack_task_inst_type(task_inst_data.task_inst_type),
  }

  and unpack_task_inst_type =
      (task_inst_type: Task_t.task_inst_type): task_inst_type =>
    switch (task_inst_type) {
    | `Reminder => Reminder
    | `Reminder_quota_counting(quota) =>
  ...
  and unpack_task_seg = ((id, size)) => (
    unpack_task_seg_id(id),
    Misc_utils.int64_of_int32_int32(size),
  )
...
module To_string = {
  let debug_string_of_sched_req_template =
      (
        ~indent_level=0,
    ...
    List.iter(
      x =>
        Sched_req_data_unit_skeleton.To_string.debug_string_of_sched_req_data_unit_skeleton(
          ~buffer,
          ~indent_level,
  ...
  let debug_string_of_arith_seq =
      (~indent_level=0, ~buffer=Buffer.create(4096), arith_seq) => {
  ...
  let debug_string_of_task =
      (~indent_level=0, ~buffer=Buffer.create(4096), (id, data)) => {
    Debug_print.bprintf(
      ~indent_level,
      buffer,
      "task id : %s\n",
  ...
  let debug_string_of_task_seg =
      (~indent_level=0, ~buffer=Buffer.create(4096), (id, size)) => {
    Debug_print.bprintf(
      ~indent_level,
      buffer,
      "task seg id : %s\n",
  ...
  let debug_string_of_task_seg_place =
      (~indent_level=0, ~buffer=Buffer.create(4096), (id, start, end_exc)) => {
    Debug_print.bprintf(
      ~indent_level,
      buffer,
      "task seg id : %s\n",
  ...
  let debug_string_of_progress =
      (~indent_level=0, ~buffer=Buffer.create(4096), progress) => {
...
module Print = {
  let debug_print_arith_seq = (~indent_level=0, arith_seq) =>
    print_string(
      To_string.debug_string_of_arith_seq(~indent_level, arith_seq),
    );
  ...
  let debug_print_task_inst = (~indent_level=0, task_inst) =>
    print_string(
      To_string.debug_string_of_task_inst(~indent_level, task_inst),
    );

  let debug_print_task_seg = (~indent_level=0, task_seg) =>
    print_string(
      To_string.debug_string_of_task_seg(~indent_level, task_seg),
    );
  ...
  let debug_print_progress = (~indent_level=0, progress) =>
    print_string(
      To_string.debug_string_of_progress(~indent_level, progress),
    );
};
