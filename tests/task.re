...
module Qc = {
  ...
  let unpack_is_inverse_of_pack_arith_seq =
    QCheck.Test.make(
      ~count=10_000, ~name="unpack_is_inverse_of_pack_arith_seq", arith_seq, x =>
      x
      |> Daypack_lib.Task.Serialize.pack_arith_seq
  ...
  let unpack_is_inverse_of_pack_task =
    QCheck.Test.make(
      ~count=10_000, ~name="unpack_is_inverse_of_pack_task", task, x =>
      x
      |> Daypack_lib.Task.Serialize.pack_task
  ...
  let unpack_is_inverse_of_pack_task_inst =
    QCheck.Test.make(
      ~count=10_000, ~name="unpack_is_inverse_of_pack_task_inst", task_inst, x =>
      x
      |> Daypack_lib.Task.Serialize.pack_task_inst
      |> Daypack_lib.Task.Deserialize.unpack_task_inst == x
    );

  let unpack_is_inverse_of_pack_task_seg =
    QCheck.Test.make(
      ~count=10_000, ~name="unpack_is_inverse_of_pack_task_seg", task_seg, x =>
      x
      |> Daypack_lib.Task.Serialize.pack_task_seg
      |> Daypack_lib.Task.Deserialize.unpack_task_seg == x
    );
